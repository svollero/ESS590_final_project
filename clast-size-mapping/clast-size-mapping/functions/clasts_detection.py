...
def clasts_detect(mode, imdirectory, imlist, resolution=0.001, metric_cropsize=1, plot=True, saveplot=False, saveresults=False, devicemode="gpu", devicenumber=0, kstart=0, ksaveint=1):
    ...
    print(imlist)
    for im in range(0, np.shape(imlist)[0]):
        try:
            imgpath=os.path.join(imdirectory,imlist[im])
...
#                         ax_list=np.zeros([n_detected_objects,2])
                    for i in range(0, n_detected_objects):
                        ...
                        axes = op.ellipse_axis_length(ell) #results = radius !!
                        a, b = axes
                        ...
                        #if a>b:
                            #clasts['Orientation'].iloc[i]=np.rad2deg(phi)
                        #else:
                            #clasts['Orientation'].iloc[i]=np.rad2deg(phi)+90
                        clasts['Score'].iloc[i]=r['scores'][i]
                    ...
                    print('-------------------------------------------------------')
                    print(' ')
                    clasts = clasts[clasts['Clast_length'].isnull()==False]
                    clasts = clasts[clasts['Clast_length']>0]
                    clasts = clasts[clasts['Clast_width']>0]
                    clasts.clast_ID.iloc[0:np.shape(clasts)[0]] = range(1, np.shape(clasts)[0]+1)
                    clasts = clasts.reset_index(drop=True)
                    if saveresults==True:
                        clasts.to_csv(imgpath[0:-4]+'_individual_clasts.csv', index=False, float_format='%.5f')
            ...
            if str.lower(mode)=="uav":
                ...
                expected_maximum_number_of_detected_objects=n_crops*300    #30 is an empiricaly expected average number of objects per image
                ...
                for n in range(0, int(shpimg[0]/cropsize)+1): #Lines
                    for m in range(0, int(shpimg[1]/cropsize)+1): #Columns
                        ...
                        if (np.shape(np.unique(croppedimage))[0]<3):
                ...
                k_id = np.linspace(0, np.shape(work_tiles_nm)[0], np.shape(work_tiles_nm)[0])
                k_pct = np.linspace(0, 100, np.shape(work_tiles_nm)[0])
                k_save_pct = np.array(range(ksaveint, 100-ksaveint+1, ksaveint))
                k_save = np.interp(k_save_pct, k_pct, k_id).astype(int)
                ...
                for k in range(kstart, np.shape(work_tiles_nm)[0]):
                    ...
                    print('Processing crops: '+'row = '+str(int(n))+', col = '+str(int(m))+' - '+str(np.round(k_pct[k],3))+' % - '+str(int(k))+' / '+str(np.shape(work_tiles_nm)[0]-1)+' crops processed')
                    ...
                    if not not(k_save[k_save==k]):
                        if saveresults==True:
                            ...
                            #clasts_temp.to_csv(imgpath[0:-4]+'_checkpoint_save_'+str(int(((k+1)/(np.shape(work_tiles_nm)[0]+1))*100))+'_%_k='+str(k)+'_window_size='+str(metric_cropsize)+'m_individual_clast_values.csv', index=False, float_format='%.5f')
                            #print(imgpath[0:-4]+'_checkpoint_save_'+str(int(((k+1)/(np.shape(work_tiles_nm)[0]+1))*100))+'_%_k='+str(k)+'_window_size='+str(metric_cropsize)+'m_individual_clast_values.csv')
                            clasts_temp.to_csv(imgpath[0:-4]+'_checkpoint_save_'+str(np.around(k_pct[k]))+'_%_k='+str(k)+'_window_size='+str(metric_cropsize)+'m_individual_clast_values.csv', index=False, float_format='%.5f')
                            print(imgpath[0:-4]+'_checkpoint_save_'+str(np.around(k_pct[k]))+'_%_k='+str(k)+'_window_size='+str(metric_cropsize)+'m_individual_clast_values.csv')
                ...
                if saveresults==True:
                    ...
                    lines = ['Start time: '+ str(start_time), 'Elapsed time: '+ str(time_elapsed)]
                    with open(imgpath[0:-4]+'_window_size='+str(metric_cropsize)+'m_detection_log.txt', 'w') as f:
                        for line in lines:
                            f.write(line)
                            f.write('\n')
